...
{
    internal sealed class FileServiceRentedArrayClientCalls: IClientCalls
    {
        private readonly ChannelBase _channel;
        private readonly IFileServiceRentedArray _fileService;

        public FileServiceRentedArrayClientCalls(ChannelBase channel, IFileServiceRentedArray fileService)
        {
            _channel = channel;
        ...
        public async Task UploadFileAsync(string filePath, int bufferSize, CancellationToken token)
        {
            ...
            var stream = File.OpenRead(filePath).ToAsyncEnumerableRented(token, progress.Report, bufferSize);
