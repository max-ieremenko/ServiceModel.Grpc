...
namespace ServiceModel.Grpc.DesignTime.CodeAnalysis.TestApi;
...
internal static class CSharpCompilationExtensions
{
    ...
    public static CSharpCompilation Create(SyntaxTree syntaxTree) => CreateCompilation(syntaxTree);
    ...
    private static CSharpCompilation CreateCompilation(SyntaxTree? syntaxTree = null)
    {
        ...
        var loadedByName = new Dictionary<AssemblyName, Assembly>(new AssemblyNameComparer());
        foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies().Where(i => !i.IsDynamic && !i.ReflectionOnly))
        {
            if (!loadedByName.ContainsKey(assembly.GetName()) && !string.IsNullOrEmpty(assembly.Location))
            {
                loadedByName.Add(assembly.GetName(), assembly);
            }
        }

        var requiredAssemblies = AppDomain
            .CurrentDomain
            .GetAssemblies()
            .Where(i => "netstandard".Equals(i.GetName().Name, StringComparison.CurrentCulture))
            .Select(i => i.GetName());
        ...
        var locations = requiredAssemblies
            .Concat(referencedAssemblies)
            .Distinct(new AssemblyNameComparer())
            .Select(i => loadedByName.TryGetValue(i, out var assembly) ? assembly : Assembly.Load(i))
            .Select(i => i.Location);

        var assemblies = new HashSet<string>(locations, StringComparer.Ordinal)
        {
    ...
    {
        public bool Equals(AssemblyName? x, AssemblyName? y) => string.Equals(x?.Name, y?.Name, StringComparison.OrdinalIgnoreCase);
