...
internal sealed class EndpointSelfHostBinderBaseBindCodeGenerator : ICodeGenerator
{
    private readonly ContractDescription<ITypeSymbol> _contract;
    private readonly bool _isStaticClass;

    public EndpointSelfHostBinderBaseBindCodeGenerator(ContractDescription<ITypeSymbol> contract, bool isStaticClass)
    {
        _contract = contract;
        _isStaticClass = isStaticClass;
    }

    public string GetHintName() => Hints.EndpointSelfHostBinderBaseBind(_contract.BaseClassName);
    ...
    public void Generate(ICodeStringBuilder output)
    {
        ...
        output
            .WriteTypeName("Grpc.Core", "ServiceBinderBase")
            .AppendLine(" serviceBinder, IServiceProvider serviceProvider, Action<global::Grpc.Core.ServiceModelGrpcServiceOptions> configure = default)")
            .AppendLine("{");
