...
if (-not $SkipLinuxSdk) {
    ...
    #docker run -it --rm --entrypoint pwsh -v "$(Join-Path $env:USERPROFILE .nuget\packages):/root/.nuget/packages" -v "$(Join-Path (Get-Location) ..\):/repository" service-model-grpc/sdk:10.0-noble
    docker run `
        -it `
        --rm `
        --entrypoint pwsh `
