...
try {
    ...
    Write-Host "=== restore ==="
    Exec { docker exec -it $containerId dotnet restore /examples/ServerFilters }

    Write-Host "=== build ==="
    Exec { docker exec -it $containerId dotnet build --configuration Release /examples/ServerFilters }
}
finally {
...
$containerId = Exec {
    docker run -d -it -v "${examplesDir}:/examples" mcr.microsoft.com/dotnet/sdk:5.0
}
...
try {
    ...
    foreach ($app in $apps) {
        Write-Host "=== exec $app ==="
        Exec { docker exec -it $containerId dotnet "/examples/ServerFilters/$app/bin/Release/net5.0/$app.dll" }
    }
}
finally {
    Exec { docker container rm -f $containerId }
